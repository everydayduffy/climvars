#' bio19: Calculate precipitation of the coldest quarter.
#'
#' @description `bio19` is used to calculate the total precipitation in the
...
#' value.
#'
#' @return a single numeric value of total precipitation in the coldest quarter
#' of the year.
#' @export
#'
...
#' summed temperature is calculated. Once the coldest 91 day period is
...
bio19 <- function(temps, prec, tme1, tme2) {
  if (is.na(sd(prec, na.rm = TRUE)) | is.na(sd(temps, na.rm = TRUE)))
    ...
    cold_id <- which(sum_temps == min(sum_temps, na.rm = TRUE))[1]
    prec2 <- c(prec,prec)
    pcold <- sum(prec2[cold_id:(cold_id + period * time_step)], na.rm = TRUE)
  }
  return(pcold)
}
